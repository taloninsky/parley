...
|-------|--------|---------|
| `/conversation/init` | POST | Create the session. Resolves persona → model → provider; for `LlmProviderTag::Anthropic` the proxy resolves the API key from its keystore via `SecretsManager` (using the optional `credential` field on the request body, default `"default"`) and constructs an `AnthropicLlm`. The literal key is *never* accepted from the request. Returns the initial `ConversationSession` JSON. |
...
| `/conversation/load` | POST | Body: `{ session_id, credential? }`. Reads the session from disk, re-resolves its active persona/model against the current registries, builds a fresh provider by resolving the named credential (default `"default"`) via `SecretsManager`, and installs a new orchestrator. Returns the loaded `ConversationSession`. |
...
**Provider construction:** Only `LlmProviderTag::Anthropic` is wired in this slice. Other tags surface as `501 Not Implemented` from `/init`. The Anthropic key is resolved server-side via `SecretsManager` (env var first, then OS keystore) and lives only inside the constructed `AnthropicLlm`; it is never accepted on the wire and never logged. When no credential is configured, `/init`, `/load`, and `/switch` return `412 Precondition Failed` with `{error:"provider_not_configured", provider, credential}`.
...
**Credential policy:** API keys live exclusively in the proxy's OS keystore (managed by `proxy::secrets::SecretsManager`, see [Secrets storage](#secrets-storage-phase-5)). Sessions on disk reference providers by tag, not by key, so session files remain safe to commit, sync, or share. Clients (browser or otherwise) only ever name a credential by its short label (`default` or a `[a-z0-9_-]{1,32}` slug); the literal secret never crosses the HTTP boundary in either direction.
...
- `proxy::providers::REGISTRY` — static catalog of every provider known to the build, grouped by `ProviderCategory` (`Stt`, `Llm`, `Tts`). Each entry pins the env-var name, the keystore service prefix, and the human-readable display name. Adding a provider is a one-line change here.
- `proxy::secrets::SecretsManager` — owns a `KeyStore` (the OS keystore via the `keyring` crate in production; an in-memory map in tests) plus an `EnvLookup` (process env in production, a static map in tests). Resolves credentials with env-var precedence: if `PARLEY_<PROVIDER>_API_KEY` is set, that wins for the `default` credential and the keystore entry is shadowed (a warning surfaces in the status payload). Named credentials (`[a-z0-9_-]{1,32}`) come exclusively from the keystore.
- `proxy::secrets::CredentialIndex` — atomically-written JSON file at `~/.parley/credentials.json` that tracks which named credentials exist per provider. The keystore itself is opaque to enumeration; the index is the source of truth for "what credentials does this provider have?"
- `proxy::secrets_api` — HTTP routes mounted under `/api/secrets`:
  - `GET /api/secrets/status` returns the categorized provider list with per-credential status (`configured`, `source: env|keystore`, optional `warning`). The actual key value never appears in any response body.
  - `PUT /api/secrets/{provider}/{credential}` sets/replaces a key (body: `{"key":"..."}`).
...
- Context compaction
- Session persistence: file-per-session JSON store landed in Phase 4d. Auto-save after each AI turn landed in **Phase 4f.1** (frontend POSTs `/conversation/save` after every `AiTurnAppended`). Session deletion landed in **Phase 4f.5** (`SessionStore::delete` + `DELETE /conversation/sessions/{id}` + frontend Delete button). **Deferred:** richer listing metadata, encryption at rest.
- Frontend wiring: ✅ landed in **Phase 4f**. `src/ui/conversation.rs` (`ConversationView`) talks to the proxy via hand-rolled `web_sys::Request` + `wasm_bindgen_futures::JsFuture`. On mount it fetches `/personas`, `/models`, and `/conversation/sessions` to populate dropdowns; on send it lazily POSTs `/conversation/init`, then POSTs `/conversation/turn` and consumes the SSE response by reading `Response::body()` as a `ReadableStreamDefaultReader`, decoding chunks with `TextDecoder`, splitting on `\n\n` event boundaries, and parsing each frame's `data:` line into a `WireEvent` (mirror of `OrchestratorEvent`). `Token { delta }` accumulates into a streaming "in-progress" assistant bubble; `AiTurnAppended` finalizes the bubble and triggers auto-save; `Failed` surfaces as a status banner. The session picker (Phase 4f.3) provides Load/New buttons backed by `/conversation/load`; loaded sessions lock the persona/model dropdowns. Phase 4f.4 added title derivation: the proxy's `/conversation/sessions` endpoint returns `{id, title}` pairs where `title` is the first user turn (whitespace-collapsed, ≤60 chars) so the picker reads as human-friendly previews instead of raw filename ids. `src/ui/root.rs` (`Root`) is a thin shell with a two-button toggle that picks between the existing transcription view (`App`) and `ConversationView`; both subtrees render simultaneously with `display:none` toggling so tab switches preserve in-progress state (Phase 4f.1). Anthropic credentials are no longer carried by a cookie: the conversation view subscribes to `/api/secrets/status` via the shared `crate::ui::secrets` client and renders a credential dropdown whose selected name is sent as the `credential` field on `/init`/`/load`/`/switch`. The proxy resolves the key from its keystore (Phase 5).
  - **Deferred frontend pieces:** transcription→conversation handoff, multi-speaker turn metadata, audio integration (TTS playback, mic capture), OpenAI provider, encryption at rest.
  - **Phase 4f.8 — retry/dismiss on Failed:** on dispatch failure the orchestrator leaves the user turn in the session (it's appended before the LLM call). `ConversationSession` gained `has_pending_user_turn` + `discard_pending_user_turn` helpers. The orchestrator's dispatch loop was factored into `dispatch_history` + `resolve_active`; a new `retry_pending` re-runs the LLM over the existing history without re-appending, and `discard_pending` pops the orphan user turn. Two new proxy routes expose them: `POST /conversation/retry` (streams SSE like `/turn` minus `user_turn_appended`, 409 when no pending turn) and `POST /conversation/discard_pending` (204/409). New `OrchestratorError::NoPendingTurn` variant maps to 409. Frontend renders Retry + Dismiss buttons inside the status row whenever `SendStatus::Failed` is active; Retry calls `/retry` and consumes the SSE stream via the shared `consume_turn_response` helper (extracted from `submit_turn`), Dismiss POSTs `/discard_pending` and pops the trailing user `Message` from the view.
  - **Phase 4f.7 — per-turn cost + running total:** `OrchestratorEvent::AiTurnAppended` already carried `cost: Cost`; the frontend now decodes it (mirrored as `WireCost { usd }`) and stores it on each assistant `Message` as `cost_usd: Option<f64>`. Loaded sessions surface historical costs via `WireTurn.provenance.cost`. Each assistant bubble renders its USD cost beneath the body; the status row shows a right-aligned running total summed across all `cost_usd` values currently in view.
  - **Phase 4f.6 — mid-session persona/model switch:** `POST /conversation/switch` now rebuilds the orchestrator: it snapshots the active session, validates the new persona/model against the registries, builds a fresh provider by resolving the request body's `credential` (default `"default"`) via `SecretsManager`, applies `ConversationSession::switch_persona` to the snapshot, and replaces `state.inner` with a new `ConversationOrchestrator` bound to the new provider map. This is required because `OrchestratorContext.providers` is per-instance and only contains the provider built at init/load time — switching to a different model would otherwise fail at the next turn dispatch with `NoProvider`. The frontend (`src/ui/conversation.rs`) keeps the persona/model dropdowns editable post-init and exposes a `Switch` button that becomes enabled when the dropdown selection diverges from the session's currently-applied persona/model (tracked via `applied_persona`/`applied_model` signals updated on init/load/switch success).
