...
- Frontend wiring: ✅ landed in **Phase 4f**. `src/ui/conversation.rs` (`ConversationView`) talks to the proxy via hand-rolled `web_sys::Request` + `wasm_bindgen_futures::JsFuture`. On mount it fetches `/personas`, `/models`, and `/conversation/sessions` to populate dropdowns; on send it lazily POSTs `/conversation/init`, then POSTs `/conversation/turn` and consumes the SSE response by reading `Response::body()` as a `ReadableStreamDefaultReader`, decoding chunks with `TextDecoder`, splitting on `\n\n` event boundaries, and parsing each frame's `data:` line into a `WireEvent` (mirror of `OrchestratorEvent`). `Token { delta }` accumulates into a streaming "in-progress" assistant bubble; `AiTurnAppended` finalizes the bubble and triggers auto-save; `Failed` surfaces as a status banner. The session picker (Phase 4f.3) provides Load/New buttons backed by `/conversation/load`; loaded sessions lock the persona/model dropdowns. Phase 4f.4 added title derivation: the proxy's `/conversation/sessions` endpoint returns `{id, title}` pairs where `title` is the first user turn (whitespace-collapsed, ≤60 chars) so the picker reads as human-friendly previews instead of raw filename ids. `src/ui/root.rs` (`Root`) is a thin shell with a two-button toggle that picks between the existing transcription view (`App`) and `ConversationView`; both subtrees render simultaneously with `display:none` toggling so tab switches preserve in-progress state (Phase 4f.1). The `parley_anthropic_key` cookie is read on mount and written back on edit, reusing the same key the transcription view uses for Haiku formatting.
  - **Deferred frontend pieces:** transcription→conversation handoff, multi-speaker turn metadata, audio integration (TTS playback, mic capture), OpenAI provider, encryption at rest, retry/skip UI on `Failed` events, per-turn cost + running total.
