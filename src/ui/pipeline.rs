...
//!   AssemblyAI, Soniox, and xAI sessions). The standalone transcription
//!   view still reads its legacy key until that surface is migrated.

use crate::ui::app;

/// Cookie key for the selected STT provider. Values: `assemblyai`,
/// `soniox`, `xai`. Anything else is treated as the default.
pub const STT_PROVIDER_KEY: &str = "parley_pipeline_stt";

const LEGACY_STT_PROVIDER_KEY: &str = "parley_stt_provider";
...
pub fn stt_provider() -> String {
    app::load(STT_PROVIDER_KEY)
        .or_else(|| app::load(LEGACY_STT_PROVIDER_KEY))
        .filter(|provider| matches!(provider.as_str(), "assemblyai" | "soniox" | "xai"))
